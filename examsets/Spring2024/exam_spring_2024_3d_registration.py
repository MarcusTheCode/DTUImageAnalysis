...
def do_3d_registration_f_2024():
    vol_template = sitk.ReadImage('T1_brain_template.nii.gz')
    ...
    plt.show()
    sitk.WriteImage(vol_moving, 'T1_brain_moving.nii.gz')
...
def otsu_on_image_f_2024():
    vol_template = sitk.GetArrayFromImage(sitk.ReadImage('T1_brain_template.nii.gz'))
    ...
    sitk.WriteImage(mask_sitk, 'T1_brain_mask.nii.gz')

    # Compute the normalized correlation coefficient between the template and moving images
    icc = normalized_correlation_coefficient(sitk.ReadImage('T1_brain_template.nii.gz'),
                                             sitk.ReadImage('T1_brain_moving.nii.gz'),
                                             sitk.ReadImage('T1_brain_mask.nii.gz'))
